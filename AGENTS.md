...
1. **班次模板**：右上角 + 添加一栏（对话框填名称 + 时间滚轮 + 色板，确定/取消）；左下角「跳过设置」（灰）+ 右下角「下一步」（高亮主按钮）。跳过后进首页并置 `onboardingDone=true`，不再强制引导。
2. **周期与逐日指派（合并为一步）**：文本框输入周期天数（1–99）；输入后下方列出"第 1..N 天"，单击某天弹出班次选择（仅列用户已定义模板）。
3. **开始日期**：年/月/日三个滚轮，默认今天，"重置为今天"按钮 → 完成进首页。
...
- **二维码**：内容是紧凑载荷（见下），下方提示对方用「设置 → 导入方案 → 扫码」扫。
...
- **导入**：设置页 →「导入方案」弹窗，**竖排四个按钮、点了直接导入**（没有文本框和二次确认）：
  「从剪贴板导入」/「扫码导入」（zxing 相机页）/「从文件导入」（`OpenDocument` 选 .json）/「取消」（着重色）。合并规则：
  班次按「名称+时间+颜色」去重复用，方案按「名称+周期+锚点+日序」去重跳过，同名不同内容自动改名；
  本机已有启用方案时不动它，本机没有启用方案才自动启用导入的方案（`domain/PlanImport.kt`，有单测）。
- 二维码生成用 ZXing（`share/QrCode.kt`：`encodeQrMatrix` 纯 JVM 可测 + `encodeQrCode` 转 Bitmap），扫码用 `zxing-android-embedded` 的 `ScanContract`（自带相机页与权限请求，Manifest 里声明了 CAMERA）。
...
### 倒班方案页（`ui/scheme/PlanEditScreen.kt`，二级页面）
一个页面管完三块，顺序固定：**上=班次模板、中=开始日期、下=周期与指派**。
- 班次模板：直接复用引导向导的 `TemplatesStep`（`internal`，带 title/hint 参数），增删改即时落库。
- 开始日期：复用向导的 `AnchorStep`（年/月/日滚轮 + 重置为今天）。
- 周期与指派：周期天数输入框 + 每天的指派行（复用 `AssignmentRow`；未指派用 `UNASSIGNED = 0L` 占位，
  因为 `Scheme.dayTemplateIds` 没留 null）。
- **编辑的是当前启用的方案，改动即时生效**（不再走三步向导，也不再"修改=生成新方案"）。
  首次启动引导仍然是原来的三步向导（`ui/onboarding/OnboardingScreen.kt`）。
- 方案列表 / 新建 / 删除放在右上角列表按钮的弹窗里（切换即改 `activeSchemeId`）。
...
- `data/Model.kt`：`ShiftTemplate` / `Scheme` / `PlanDocument`（kotlinx-serialization）。
...
- 二级页面走 `PageCardStack`：底层标签页**原地不动只压暗**，卡片从右侧整页滑入（前段 32dp 圆角、贴合时归零）。
...
- 引导向导页：步骤内容放 `Box(Modifier.weight(1f))`、底部按钮行放 weight 之外——CycleAssignStep 是 LazyColumn，
  不加约束会吃光 Column 剩余高度，把"上一步/下一步"挤出屏幕。
...
- **Dialog 规范**：按钮顺序 `not_modified | cancel | confirm`（三按钮 weight(1f) + `spacedBy(8.dp)`，confirm 用 `textButtonColorsPrimary()`）。
  ...
  **表单类弹窗的字段要在"打开时"重置**：常驻组合意味着 `remember(existing)` 不会因为关闭而丢弃——
  新建（`existing` 一直是 null，key 不变）会残留上一次填的内容，编辑同一个对象也会残留上次取消掉的改动。
  用 `LaunchedEffect(show, existing) { if (show) { …重新赋值… } }` 重置（**只在 show 为 true 时重置**，
  否则退出动画期间会闪回初始值）；`remember(key)` 当成初始值保留即可。
